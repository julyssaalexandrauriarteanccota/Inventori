...
async function main() {
  ...
  await send(`T1-NoExtraNS`, sign(xml(`B001-${String(n++).padStart(8,'0')}`, ''), pk, cert), `${RUC}-03-B001-${String(n-1).padStart(8,'0')}.xml`);

  // Test 2: Only xsi + xsd
  await send(`T2-xsi+xsd`, sign(xml(`B001-${String(n++).padStart(8,'0')}`, 'xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" '), pk, cert), `${RUC}-03-B001-${String(n-1).padStart(8,'0')}.xml`);

  // Test 3: Only ccts + qdt + udt
  await send(`T3-ccts+qdt+udt`, sign(xml(`B001-${String(n++).padStart(8,'0')}`, 'xmlns:ccts="urn:un:unece:uncefact:documentation:2" xmlns:qdt="urn:oasis:names:specification:ubl:schema:xsd:QualifiedDatatypes-2" xmlns:udt="urn:un:unece:uncefact:data:specification:UnqualifiedDataTypesSchemaModule:2" '), pk, cert), `${RUC}-03-B001-${String(n-1).padStart(8,'0')}.xml`);

  // Test 4: All 5 extra namespaces
  await send(`T4-AllExtraNS`, sign(xml(`B001-${String(n++).padStart(8,'0')}`, 'xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:ccts="urn:un:unece:uncefact:documentation:2" xmlns:qdt="urn:oasis:names:specification:ubl:schema:xsd:QualifiedDatatypes-2" xmlns:udt="urn:un:unece:uncefact:data:specification:UnqualifiedDataTypesSchemaModule:2" '), pk, cert), `${RUC}-03-B001-${String(n-1).padStart(8,'0')}.xml`);

  // Test 5: Only xsi
  await send(`T5-only-xsi`, sign(xml(`B001-${String(n++).padStart(8,'0')}`, 'xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" '), pk, cert), `${RUC}-03-B001-${String(n-1).padStart(8,'0')}.xml`);
