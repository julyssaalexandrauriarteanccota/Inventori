...
async function sendTest(label: string, ruc: string, signedXml: string, xmlFileName: string) {
  console.log(`\n===== ${label} =====`);
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ser="http://service.sunat.gob.pe" xmlns:wsse="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd">
  ...
  const response = await fetch(BETA_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'text/xml; charset=utf-8', SOAPAction: 'urn:sendBill' }, body: envelope });
  console.log(`HTTP Status: ${response.status}`);
  const text = await response.text();
  const faultCode = text.match(/<faultcode>(.*?)<\/faultcode>/)?.[1];
  const faultString = text.match(/<faultstring>(.*?)<\/faultstring>/)?.[1];
  if (faultCode) { console.log(`FAULT: ${faultCode}`); console.log(`MSG: ${faultString}`); }
  else { console.log(`Response: ${text.slice(0, 500)}`); }
}
...
async function main() {
  ...
  await sendTest('Real RUC Boleta', REAL_RUC, signed1, `${REAL_RUC}-03-B001-00000085.xml`);
  ...
  await sendTest('Real RUC Factura', REAL_RUC, signedF, `${REAL_RUC}-01-F001-00000085.xml`);
  ...
  await sendTest('Test RUC 20000000001', TEST_RUC, signed3, `${TEST_RUC}-03-B001-00000085.xml`);

  // Test 4: Try with encoding=UTF-8 instead of ISO-8859-1
  const xml4 = buildBoletaXml(REAL_RUC, 'B001-00000084').replace('encoding="ISO-8859-1"', 'encoding="UTF-8"');
  const signed4 = signXml(xml4, privateKeyPem, certificatePem);
  await sendTest('UTF-8 encoding', REAL_RUC, signed4, `${REAL_RUC}-03-B001-00000084.xml`);

  // Test 5: Try removing standalone="no"
  const xml5 = buildBoletaXml(REAL_RUC, 'B001-00000083').replace(' standalone="no"', '');
  const signed5 = signXml(xml5, privateKeyPem, certificatePem);
  await sendTest('No standalone', REAL_RUC, signed5, `${REAL_RUC}-03-B001-00000083.xml`);
  ...
  await sendTest('schemeAgencyName on CustomizationID', REAL_RUC, signed6, `${REAL_RUC}-03-B001-00000082.xml`);
