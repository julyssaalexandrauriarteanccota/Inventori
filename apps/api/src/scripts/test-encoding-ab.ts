/**
 * Quick test: compare EXACT same XML except for attributes on InvoiceTypeCode and DocumentCurrencyCode
 */
...
async function main() {
  ...
  // MINIMAL XML - exact copy of what fails with 2074, but UTF-8 encoding
...
<Invoice xmlns="urn:oasis:names:specification:ubl:schema:xsd:Invoice-2" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2">
  ...
  // SAME as minimal but with ISO-8859-1 encoding
  const xmlISO = xmlMinimal.replace('encoding="UTF-8"', 'encoding="ISO-8859-1" standalone="no"');

  await send('T1 - Minimal UTF-8', sign(xmlMinimal, pk, cert), `${RUC}-03-B001-00000060.xml`);
  await send('T2 - Minimal ISO-8859-1', sign(xmlISO, pk, cert), `${RUC}-03-B001-00000061.xml`);
