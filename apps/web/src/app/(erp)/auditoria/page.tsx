...
export default function AuditoriaPage() {
  ...
  const autoRefresh = usePageAutoRefresh({
    scope: "auditoria",
    toastLabel: "Auditoría",
    manualToastMessage: "Auditoría actualizada",
    toastId: AUDITORIA_REFRESH_TOAST_ID,
  });
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-sm font-medium">
            {formatModelo(row.original.modelo)}
          </span>
        ),
      },
      {
          ...
          row.original.usuario ? (
            <div className="flex flex-col max-w-[220px]">
              <span
                ...
                title={row.original.usuario.nombre}
              >
                {row.original.usuario.nombre}
              </span>
              <span
                className="truncate text-xs text-muted-foreground"
                title={row.original.usuario.email}
              >
