...
> **Primary source of truth:** [`AGENTS.md`](AGENTS.md). If anything below conflicts with `AGENTS.md`, follow `AGENTS.md`. Scoped rules in [`.github/instructions/`](.github/instructions/) apply automatically to their respective paths (`apps/api/src/**`, `apps/ai/**/*.py`, `apps/api/prisma/**`, `apps/web/**`).

---

## What This Project Is

**Inventori** is a full-stack ERP for office equipment companies (copiers/printers), built as a `pnpm` monorepo. Three runtime services + shared package:
...
| `apps/ai` | FastAPI + Python 3.12 (OCR + ticket classification) | 8000 | **no** — standalone |
| `packages/shared` | Enums, Zod schemas, TS types | — | yes (`@erp/shared`) |

`pnpm-workspace.yaml` only includes `apps/api`, `apps/web`, `packages/*`. `apps/ai` lives outside the workspace and is started by `pnpm dev:ai` via its `.venv`.
...
pnpm dev               # Runs web + api + ai concurrently
```

Individual services: `pnpm dev:web` / `pnpm dev:api` / `pnpm dev:ai`

`apps/ai` requires a pre-built venv at `apps/ai/.venv` — `pnpm dev:ai` literally invokes `.venv\Scripts\python.exe` on Windows.
...
CI toolchain: Node 24, pnpm 10, Python 3.12. CI order: `lint → type-check → tests → build`. Only `apps/api` lint runs `eslint --fix`; web and shared do not auto-fix.
...
cd apps/ai && pytest --tb=short   # Python tests
```
...
Browser → Next.js (3000) → NestJS API (4000) → PostgreSQL / Redis / MinIO
                                              → FastAPI AI (8000)  [API → AI only]
```

The web layer **never** calls the AI service directly — it always routes through the API. The API forwards to `apps/ai` with `X-Internal-Key` header; `apps/ai` rejects any non-`/health` request without that header.
...
### NestJS API modules (`apps/api/src/modules/`, 25 modules)
...
- All Pydantic models, no raw dicts
- Only `claude_service.py` imports `anthropic` — falls back to mock responses when `ANTHROPIC_API_KEY` is absent
- Validate `X-Internal-Key` on every endpoint except `/health`
- Endpoints: `POST /ocr/invoice`, `POST /ocr/serial`, `POST /clasificar/ticket`, `GET /health`
- Heavy tasks via Celery + Redis (workers folder exists but currently empty)
...
| `.env` (root) | Shared by all three services. NestJS loads via `ConfigModule.forRoot({ envFilePath: '../../.env' })`; FastAPI via `SettingsConfigDict(env_file='../../.env')` |
...
| `.github/instructions/` | Auto-applied scoped rules (NestJS, FastAPI, Prisma, Next.js) |
...
- [`apps/ai/README.md`](apps/ai/README.md) — FastAPI endpoints, Claude integration, env vars
